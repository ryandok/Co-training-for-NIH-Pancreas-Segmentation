"""
Training multi-view co-training 3D U-Net version 3
"""
# TODO: dataloader
# TODO: two model
# TODO: loss
...
parser.add_argument("--batch_size",
                    type=int,
                    default='3',
                    help="batch size for co-training")
...
parser.add_argument("--model_pre_trained_dir",
                    ...
                    default='../model/Pancreas_model/UNet3D/MultiViewsPreTrained/coronal-sagittal-axial2',
                    # default=None,
...
def train(gpu,
    ...
    Training 3D U-Net
    :param gpu: gpu id
    :param batch_size: batch size for stage 2
    ...
    train_dataset = Pancreas(train_volume_path, train_label_path, RC_patch=(128, 128, 128),
    ...
    for i in range(view_num):
        model_multi_views[i].load_state_dict(torch.load("%s/model_view%d.ckpt"% (model_pre_trained_dir, i+1)))


    optimizer_multi_views = []
    for i in range(view_num):
        optimizer_view = torch.optim.SGD(model_multi_views[i].parameters(), lr=lr, momentum=0.9, weight_decay=4e-5)
        optimizer_multi_views.append(optimizer_view)
    ...
    for epoch in range(n_epochs):
        ...
        for batch_index, sample_batch_views in enumerate(train_dataloader):
            ...
            T = 8
            unlabeled_volume_batch_repeat_views = [unlabeled_volume_batch_views[i].repeat(2, 1, 1, 1, 1) for i in
                                                       range(view_num)]
            stride = unlabeled_volume_batch_repeat_views[0].shape[0] // 2
            pred_unlabeled_bayes_views = [torch.zeros([stride * T, 2, 128, 128, 128]).to('cuda:%d' % i) for i in
                                              range(view_num)]
            for t in range(T // 2):
                noise_unlabeled_volume_views = [unlabeled_volume_batch_repeat_views[i] +
                                                    torch.clamp(
                                                        torch.rand_like(unlabeled_volume_batch_repeat_views[i]) * 0.1,
                                                        -0.2, 0.2).to('cuda:%d' % i) for i in range(view_num)]
                with torch.no_grad():
                    for i in range(view_num):
                        pred_unlabeled_bayes_views[i][2 * stride * t:2 * stride * (t + 1)] = model_multi_views[i](
            ...
            pred_unlabeled_bayes_views = [pred_unlabeled_bayes_views[i].reshape(T, stride, 2, 128, 128, 128) for i
                                              in range(view_num)]
            pred_unlabeled_bayes_views = [torch.mean(pred_unlabeled_bayes_views[i], dim=0) for i in
                                              range(view_num)]  # [view_num * (bs, 2, 128, 128, 128)]
            uncertainty_views = [
                    -1.0 * torch.sum(pred_unlabeled_bayes_views[i] * torch.log(pred_unlabeled_bayes_views[i] + 1e-6),
                                     dim=1, keepdim=True) for i in
                    range(view_num)]  # [view_num * (bs, 1, 128, 128, 128)]
            ...
            for i in range(view_num):
                if i == 0:
                    ...
                    pred_unlabeled_softmax_views[i] = pred_unlabeled_softmax_views[i].rot90(dims=rotate_axes[i-1], k=-1)
            ...
            # for i in range(view_num):
            #     # save pseudo label
            #     pseudo_label_array = pseudo_label_views[i].cpu().detach().numpy().astype(np.int16)
            #     util.save_volume(pseudo_label_array, '../data', 'pseudo_view%d.nii.gz'%(i+1))
            #     # save unlabeled prediction softmax
            #     prediction_array = pred_unlabeled_softmax_views[i].cpu().detach().numpy()
            #     prediction_array = np.argmax(prediction_array, axis=1).astype(np.int16)
            #     util.save_volume(prediction_array, '../data', 'predict_un_softmax_view%d.nii.gz' % (i + 1))
            #     # save unlabeled prediction
            #     prediction_array = pred_unlabeled_views[i].cpu().detach().numpy()
            #     prediction_array = np.argmax(prediction_array, axis=1).astype(np.int16)
            #     util.save_volume(prediction_array, '../data', 'predict_un_view%d.nii.gz'%(i+1))
            #     # save labeled prediction
            #     prediction_array = pred_labeled_views[i].cpu().detach().numpy()
            #     prediction_array = np.argmax(prediction_array, axis=1).astype(np.int16)
            #     util.save_volume(prediction_array[0], '../data', 'predict_view%d.nii.gz' % (i + 1))
            #     # save GT label
            #     prediction_array = label_batch_views[i].cpu().detach().numpy()
            #     prediction_array = prediction_array.astype(np.int16)
            #     util.save_volume(prediction_array[0], '../data', 'GT_view%d.nii.gz' % (i + 1))
            ...
            for i in range(view_num):
                ...
                loss_sup2 = criterion2(pred_labeled_softmax_views[i][:, 1, :, :, :],
            ...
            for i in range(view_num):
                ...
                loss_cot2 = criterion2(pred_unlabeled_softmax_views[i][:, 1, :, :, :],
            ...
            loss_total_views = [loss_sup_total_views[i] + 0.5 * loss_cot_total_views[i] for i in range(view_num)]
            for i in range(view_num):
            ...
            logging.info("[Epoch: %4d/%d] [n_total_iter: %5d] [labeled volume index: %2d/%d] "
                             ...
                             % (epoch, n_epochs, n_total_iter + 1, batch_index + 1, len(train_dataloader),
                                loss_total_views[0].item(), loss_total_views[1].item(), loss_total_views[2].item(),
                                time_left))

            # tensorboardX log writer
            for i in range(view_num):
                writer.add_scalar("loss_view%d/loss_SupCot_total" % i, loss_total_views[i].item(),
                                      global_step=n_total_iter)

                writer.add_scalar("loss_view%d/loss_sup_total" % i, loss_sup_total_views[i].item(),
                                      global_step=n_total_iter)
                writer.add_scalar("loss_view%d/loss_sup_CrossEntropy" % i, loss_sup1_views[i].item(),
                                      global_step=n_total_iter)
                writer.add_scalar("loss_view%d/loss_sup_Dice" % i, loss_sup2_views[i].item(),
                                      global_step=n_total_iter)

                writer.add_scalar("loss_view%d/loss_cot_total" % i, loss_cot_total_views[i].item(),
                                      global_step=n_total_iter)
                writer.add_scalar("loss_view%d/loss_cot_CrossEntropy" % i, loss_cot1_views[i].item(),
                                      global_step=n_total_iter)
                writer.add_scalar("loss_view%d/loss_cot_Dice" % i, loss_cot2_views[i].item(),
                                      global_step=n_total_iter)
            # ---------------------
            #       save model
            # ---------------------
            if n_total_iter % n_save_iter == 0:
                ...
                for i in range(view_num):
                    torch.save(model_multi_views[i].state_dict(),
