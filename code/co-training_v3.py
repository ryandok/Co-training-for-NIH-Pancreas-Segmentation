...
parser.add_argument("--batch_size_stage1",
                    type=int,
                    default='3',
                    help="batch size for stage1")

parser.add_argument("--batch_size_stage2",
                    type=int,
                    default='3',
                    help="batch size for stage2")
...
def train(gpu,
    ...
    batch_sampler = TwoStreamBatchSampler(labeled_idxs, unlabeled_idxs, batch_size_stage2, batch_size_stage2 - labeled_bs)

    train_stage1_dataset = Pancreas(train_volume_path[0:12], train_label_path[0:12],
    ...
    train_stage2_dataset = Pancreas(train_volume_path, train_label_path,
                                    transform_views=[transforms.Compose([RandomNoise(), ToTensor()]),
                                                     transforms.Compose([RandomNoise(), Rotate((0, 2)), ToTensor()]),
                                                     transforms.Compose([RandomNoise(), Rotate((1, 2)), ToTensor()])])
    ...
    train_stage1_dataloader = DataLoader(train_stage1_dataset, batch_size=batch_size_stage1, shuffle=True, num_workers=4,
                                  pin_memory=True, worker_init_fn=worker_init_fn)

    train_stage2_dataloader = DataLoader(train_stage2_dataset, batch_sampler=batch_sampler, num_workers=4,
                                  pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    for epoch in range(n_epochs):
        if n_total_iter < n_iter_view_wise:
            ...
            view-wise training
            """
            for batch_index, sample_batch_views in enumerate(train_stage1_dataloader):
                # start_time
                start = time.time()
                ...
                labeled_volume_batch_views = [sample_batch_views[i]['volume'] for i in range(view_num)]

                # [view_num * (bs, 1, 128, 128, 128)]
                label_batch_views = [sample_batch_views[i]['label']for i in range(view_num)]
                ...
                iter_left = (n_epochs - epoch) * (len(train_stage1_dataloader) - batch_index)
                ...
                logging.info("[Epoch: %4d/%d] [n_total_iter: %5d] [labeled volume index: %2d/%d] "
                             ...
                             % (epoch, n_epochs, n_total_iter + 1, batch_index + 1, len(train_stage1_dataloader),
                                loss_sup_total_views[0].item(), loss_sup_total_views[1].item(),
            ...
            co-training
            """
            for batch_index, sample_batch_views in enumerate(train_stage2_dataloader):
                # start_time
                start = time.time()
                ...
                iter_left = (n_epochs - epoch) * (len(train_stage2_dataloader) - batch_index)
                time_left = datetime.timedelta(seconds=iter_left * (end - start))

                # print log
                logging.info("[Epoch: %4d/%d] [n_total_iter: %5d] [labeled volume index: %2d/%d] "
                             "[loss view1: %f] [loss view2: %f] [loss view3: %f][ETA: %s]"
                             % (epoch, n_epochs, n_total_iter + 1, batch_index + 1, len(train_stage2_dataloader),
